...
Usage: python3 oracle_charm_strand.py [account] [password]
"""
import os
import re
import sys
...
RAW = "../../re/oracle/oracle_charm_strand.raw"
LOG = "../../re/oracle/oracle_charm_strand_timing.log"
sfx = 2
while os.path.exists(RAW):
    RAW = f"../../re/oracle/oracle_charm_strand{sfx}.raw"
    LOG = f"../../re/oracle/oracle_charm_strand{sfx}_timing.log"
    sfx += 1
...
if charmed_at is None:
    ...
    # --- STRAND TEST: teleport away (no breadcrumbs), return, repeat ------
    note("=== strand test: 40s exiles, suppression check on each return ===")
    for strand in range(6):
        sess.send(f"/xgoto {HEALER} 1", pause=1.6)
        sess.dump(40.0)
        sess.send(f"/xgoto {room} 6", pause=1.6)
        sess.dump(2.0)
        out = cmd("look", tag=f"strand {strand} return", drain=2.0)
        if "kobold" not in out:
            note(f"pet GONE after {strand+1} strand cycles "
                 f"(+{time.time()-charmed_at:.0f}s from charm)")
            break
        if not suppressed(15.0, f"strand{strand} "):
            note(f"pet HOSTILE again after {strand+1} strand cycles "
                 f"(+{time.time()-charmed_at:.0f}s) — released, not gone")
            break
        note(f"  strand {strand}: still ours")
        if time.time() - charmed_at > 220:
            note("charm nearing its ~234s expiry; strand test ends")
            break
