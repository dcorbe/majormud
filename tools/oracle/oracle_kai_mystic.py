#!/usr/bin/env python3
"""M5 slice-5 Task 1: interactive FIFO-driven session for the mystic/kai
expedition (power acquisition on train, invoke wording).

Same driver pattern as oracle_blur_duration.py, parameterized so the
expedition's multiple sessions (pre-patch creation, post-patch training)
each get their own raw capture.

Usage: oracle_kai_mystic.py <scratch-base> <raw-path>

Commands (one per line into the FIFO <scratch-base>/kai_cmd.fifo):
  send <text>   -> text + CRLF
  send          -> bare CRLF
  raw <escaped> -> unicode-escaped bytes verbatim (e.g. `raw \x08\x08`)
  quit          -> flush, close raw capture, exit

Clean log: <scratch-base>/kai_clean.log ("<epoch> RX|PART|TX <text>")
"""
import os, re, select, socket, sys, time

BASE = sys.argv[1]
RAW = sys.argv[2]
LOG = f"{BASE}/kai_clean.log"
FIFO = f"{BASE}/kai_cmd.fifo"
...
def main():
    ...
    logf.write(f"{time.time():.3f} !! driver connected raw={RAW}\n")
    while True:
