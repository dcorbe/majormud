...
/// ported as `took_item` in `mud-core::text`). "You picked up <item>"
/// (no trailing period), live in oracle_charm_lifecycle, is kept for
/// the reimplemented board this client actually plays, whose item
/// wording has never been captured. Coins are [`picked_up`]'s and are
...
### Task 8: The stop waits for a get's completion

Added after Task 6 surfaced it. The stop pump's gate goes idle on the echo of a command, and the board answers a `get` one event after its echo with "You picked up ...". With `dwell_empty_seconds` at its shipped default of 0, a stop proven empty ends in that gap, and the pickup line, plus the `i` the bot sends on it to re-read the pack, are never processed by the stop. The session still parses any `i` that does go out, but the `i` never goes out. Task 6's scripted test hid this with a test-only `dwell_empty_seconds: 1`. This task makes the stop owe a `get` its answer the same way it already owes a `look` one, and removes the workaround.

**Files:**
- Modify: `crates/mud-client/src/farm.rs` (`StopState`: the struct fields near line 700, `new` near 725, `on_sent` near 740, `invalidate` near 760, `on_event` near 823, `verdict` near 952)
- Test: `crates/mud-client/tests/farm.rs` (the `Stop` harness near line 1230 and its tests)
- Modify: `crates/mud-client/tests/farm_scripted.rs` (remove `dwell_empty_seconds: 1,` from the test `a_key_at_a_stop_is_picked_up_and_the_pack_reread`)

**Interfaces:**
- Consumes: `crate::bot::picked_up(&str) -> Option<(u32, String)>` and `crate::bot::picked_up_item(&str) -> Option<String>` from Task 5, `StopState::on_sent(&mut self, line: &str, id: CmdId)`, `Verdict::Waiting { until }`.
- Produces: nothing new outside `StopState`.

- [ ] **Step 1: Write the failing tests**

Append to `crates/mud-client/tests/farm.rs`, after the test `a_dark_answer_settles_an_outstanding_look`:
...
`answering`, `unsolicited`, `block`, `combat_bot`, `POKE_MS` and the `Stop` harness with `fold`, `feed`, `sent`, `look_and_see` and `verdict` already exist in that file.

- [ ] **Step 2: Run the tests to verify they fail**

Run: `cargo test -p mud-client --test farm -- unanswered_get overdue_get coin_pickup_settles`
Expected: the first two fail at their first `assert_ne!`, the stop returns `Empty` while the get is unanswered. The third fails the same way.

- [ ] **Step 3: Add the pending get to `StopState`**

In `crates/mud-client/src/farm.rs`, add a field to `StopState` after `pending_look`:
...
In `StopState::new`, add `pending_get: None,`.

In `on_sent`, extend the `if` chain:
...
In `invalidate`, add `self.pending_get = None;`.

In `on_event`, before the `match &cor.event {`, add:
...
In `verdict`, directly after the `pending_look` block that returns `Waiting` or `Ask`, add:
...
Run: `cargo test -p mud-client --test farm`
Expected: all pass, including the three new ones.

- [ ] **Step 5: Remove the scripted test's workaround**

In `crates/mud-client/tests/farm_scripted.rs`, in the test `a_key_at_a_stop_is_picked_up_and_the_pack_reread`, delete the line `dwell_empty_seconds: 1,` and any comment that explains it as a workaround. Then:

Run: `cargo test -p mud-client --test farm_scripted`
Expected: all pass. The stop now waits for the pickup line, the bot's `i` goes out, and the re-read reaches the pack.

- [ ] **Step 6: Run the whole workspace and rebuild**
...
```bash
git add crates/mud-client/src/farm.rs crates/mud-client/tests/farm.rs crates/mud-client/tests/farm_scripted.rs
git commit -m "fix(client): a farm stop waits for a get to be answered"
```
