...
// formats below are the visible intent with our color constants. The
// privileged-viewer Exp column is omitted (sysop surface is
// schema-only in M7 — gangs.md §7).
pub const NO_GANGS_ESTABLISHED: &str = "There are no gangs currently established!";
...
pub fn top_gangs_header() -> Vec<String> {
    vec![
        ...
        String::new(),
        format!(
            "\x1b[31mRank \x1b[32m{:<19.19} \x1b[35m{:<11.11} \x1b[33m{:>7.7} \x1b[32m{}",
            "Gangname", "Leader", "Members", "Created"
        ),
        format!(
            "\x1b[1;30m{}{}",
            "=-".repeat(37),
...
pub fn top_gang_row(rank: usize, gang: &str, leader: &str, members: u16, created: &str) -> String {
    format!(
        "\x1b[31m{:>3.3}. \x1b[32m{gang:<19.19} \x1b[35m{leader:<11.11} \x1b[33m{:>4.4}    \x1b[32m{created}{}",
        rank.to_string(),
        members.to_string(),
        color::RESET
    )
}
/// `ncedat`'s MM/DD/YY render of a wall-seconds stamp.
pub fn ncedat(secs: i64) -> String {
    // Howard Hinnant's civil-from-days, u32-safe for our range.
