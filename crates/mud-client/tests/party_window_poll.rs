...
#[tokio::test]
async fn a_member_polls_the_party_on_its_interval_without_a_note() {
    let base = std::path::PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("party_window_poll");
    ...
    let addr = poll_board(sent.clone()).await;
