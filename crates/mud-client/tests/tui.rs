...
///
/// `probe_sheet`'s spell-listing collection has no terminal wording to
/// watch for (`farm.rs`'s own doc: "No terminal wording is pinned for
/// the spell listing"), so it always waits out its full 3-second window
/// -- twice here, once for the redirected `spells` and once for
/// `powers` -- before `Session::set_sheet` is called. A text-based
/// `session.expect` on the transcript would return the instant the
/// board's reply arrives, well before that, so this waits out both
/// windows directly instead of racing them.
#[tokio::test]
async fn the_mystic_redirect_happens_once_at_login() {
    ...
    assert_eq!(
        casting,
        mud_client::sheet::Casting::Powers,
