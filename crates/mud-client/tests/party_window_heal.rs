...
/// following Beef, then has Beef say `@heal 30` twice, about five
/// seconds apart. Every line the client sends is logged.
...
#[tokio::test]
async fn a_healer_answers_a_said_request_by_name() {
    let base = std::path::PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("party_window_heal");
    ...
    loop {
        if sent.lock().unwrap().iter().filter(|l| l.as_str() == "cast mahe beef").count() >= 2 {
            break;
        }
